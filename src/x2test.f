...
C****    uses the IBM  output files for it's input files. Like IBM the *
...
C****                                                                  *
C**** extensive modification by Lines 18 JUN 97                        *
C****  Last Modification by Lines 18 Jun 97                            *
C***********************************************************************
...
C**** Calculate the extinction cross section using the optical therem **
      call sigOT2(SIGe,K3,K,R,DVOL,X,E,alpha,beta,gamma,psi,NUSE)
...
C***********************************************************************
C*---------------------------------------------------------------------*
      subroutine sigOT2(SIGe,K3,K,R,DVOL,X,E,alpha,beta,gamma,psi,NUSE)
C*---------------------------------------------------------------------*      
C***********************************************************************
...
C ***     calculate the factor that depensd on Emu,j                 ***
          do I=1,2
             FE(I)=E(mu,I)
          end do
          FE(3)=cmplx(0.0,0.0)
c          do I=1,3
c             do J=1,3
c                FE(I)=FE(I)+(delta(I,J)-Rhat(I)*Rhat(J))*E(mu,J)
c             end do      
c          end do
...
C ***     calculate the factor that depensd on Emu,j                 ***
          ...
          do I=1,3
             do J=1,3
                FE(I)=FE(I)+(delta(I,J)-Rhat(I)*Rhat(J))*E(mu,J)
             end do      
          end do
C ***     now put the scattering amplitude together                  ***
          do I = 1, 3
...
C*---------------------------------------------------------------------*      
C***********************************************************************
C*    Function to calculate the Kroneger delta                         *
C***********************************************************************
C**** Variables                                                     ****      
      integer i,j         
      delta=0.0
