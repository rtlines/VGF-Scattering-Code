...
C****   program CHI2IN).  The scattering is computed through a plane   *
...
C****                                                                  *
C**** Time dependence:                                                 *
C****   The x2 code uses the time dependence exp(-iwt).                *
C****                                                                  *
C**** Geometry:                                                        *
...
c      NK=count
      ...
      ERR=ERRlast
      print *,'last ',NK 
c      call printkv(kth,kph,NK,KFILE)
c      do i=1,NK
c         call kvectors4(i,khatN,kth,kph,NK)
c      end do
...
C *** Internal field calculation                                     ***
      ...
      open(unit=43,file='t.dat',status='UNKNOWN')
      do a=1,NUSE
         do i=1,3
            do N =1,NK
               write(43,22) KhatN(N,1),KhatN(N,2),KhatN(N,3)
               do j=1,3
                  T1(a,i,N,j)=(0.0,0.0)
                  do b=1,NUSE
                     dtemp=d(b) 
...
C
          if (kcount.lt.NK-1) then 
             call movekv(kcount+1,khatN,kth,kph,NK,iseed,divd) 
          end if
      else
          call printkv(kth,kph,NK,KFILE,ERR,ERRlast,mcount,kcount)    
          call Ecalc(NUSE,R,E,F,An,khatN,X,NK,K,mm)
...
C***********************************************************************       
C*---------------------------------------------------------------------*
      subroutine movekv(khatN,kth,kph,NK,iseed,divd)  
C***********************************************************************
C *** Set the value of NMAX via an included file                     *** 
      implicit none
      include 'nmax.inc'
C****  Variables          
      ...
      integer N,iseed ,NK
      parameter (PI=3.141592654)
c      parameter (PI2=6.28318530718)
      THinc=PI/divd
c      PHinc=2*THinc      
      do N=2,NK-1
        kth(N)=kth(N)+THinc*rannum(iseed)
c        kph(N)=kph(N)+PHinc*rannum(iseed)
        if (kth(N).gt.PI) then
           kth(N)=2*PI-kth(N)
        end if               
c        if (kph(N).gt.PI2) then
c           kph(N)=kph(N)-PI2
c        end if   
        if(kth(N).lt.0.0) then
           kth(N)=-kth(N)
        end if    
c        if(kph(N).lt.0.0) then
c           kph(N)=-kph(N)
c        end if 
      end do     
      call kvectors3(khatN,kth,kph,NK)
...
C***********************************************************************       
C*---------------------------------------------------------------------*
      subroutine kvectors(khatN,count,NTH,NPH)                      
C*---------------------------------------------------------------------* 
C*    Subroutine to set up the  khatN directions.                      *
C*    first cut, just use an equal delta theta and sin(theta) delta phi* 
C*    see Rkvectors, I am working on this one right now
C***********************************************************************       
C *** Set the value of NMAX via an included file                     *** 
      implicit none
      include 'nmax.inc'
C****  Variables          
      real khatN(KNMAX,3), TH, PH,PI ,dp
      real delTh, delPh, dpp
      integer NTH,NPH,i,j,count , Np
      parameter (PI=3.141592654)  
        count=count+1 
        Write (*,*) 'Define the Kn vectors'
C        NTH=7.0
...
c ***   Strait up first       
        Th=0      
        PH=0
        khatN(count,1)=sin(Th)*cos(Ph) 
        khatN(count,2)=sin(Th)*sin(Ph)
        khatN(count,3)=cos(Th)  
        count=count+1  
c ***   now down        
        Th=PI      
        PH=0
...
C***********************************************************************       
C*---------------------------------------------------------------------*
      subroutine kvectors2(khatN,count,NTH,NPH)                        
C*---------------------------------------------------------------------* 
C*    Subroutine to set up the  khatN directions.                      *
C*    attempt a non uniform set                                        *
C***********************************************************************       
C *** Set the value of NMAX via an included file                     *** 
      implicit none
      include 'nmax.inc'
C****  Variables          
      real khatN(KNMAX,3), THI(30),TH, PH,PI 
      real delTh, delPh
      integer NTH,NPH,i,j,count
      parameter (PI=3.141592654)  
        NTH=6
        count=count+1 
        Write (*,*) 'Define the Kn vectors'
...
c ***   now the rest        
        do i=1,NTH         
           ...
           do j=1,NPH
              ...
              khatN(count,2)=sin(Th)*sin(Ph)
              khatN(count,3)=cos(Th)                      
              print *,count, TH*180/PI,PH*180/PI
              count=count+1
           end do            
        end do
        count=count-1 
        print *, NTH, NPH, 'NTH, NPH'
        print *,'last ',count
      return
      end
C***********************************************************************       
C*---------------------------------------------------------------------*
c      subroutine Rkvectors(khatN,count)                                     
C*---------------------------------------------------------------------* 
C*    Subroutine to set up the  khatN directions.                      *
C*    first cut, just use an equal delta theta and sin(theta) delta phi*
C***********************************************************************       
...
c      include 'nmax.inc'
C****  Variables          
c      real khatN(KNMAX,3), th, ph, delPh, delTh,PI
c      integer Nvectors,i,j,count, iseed
c      parameter (PI=3.141592654) 
c        Write (*,*) 'Define the Kn vectors'
c        Nvectors=25
c        iseed=23576 
c        count=count+1
c ***   Strait up first       
c        Th=0      
c        PH=0
c        khatN(count,1)=sin(Th)*cos(Ph) 
c        khatN(count,2)=sin(Th)*sin(Ph)
c        khatN(count,3)=cos(Th)
c        count=count+1 
c ***   now down        
c        Th=PI      
c        PH=0
